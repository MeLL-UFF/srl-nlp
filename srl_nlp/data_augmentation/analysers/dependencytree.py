...
class DependencyTreeLocalAPI:
    ...
    def sentence2LF(self, sentence, source=None, id=None, *args, **kargs):
        ...
        if len(sentence.strip()) == 0:
            return []
        tree = self.parser(sentence.strip())
        out = []
...
#                 struct_dep.rel = rel
#             parent = tree.get(gov,
#                               self._Dependence('ROOT', gov, gov))  # if there is not a gov representation, create it
...
#     def _init_parser(self, model=config.get('syntactic_local', 'spacy_model'), *args, **kargs):
