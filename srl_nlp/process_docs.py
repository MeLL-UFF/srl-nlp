...
class ProcessorAbstract(object):
    ...
    *Abstract class*
    ...
    def replace_entities(self, doc, text):
        ...
        # sort by length to avoid replacing '@entity1' in '@entity12', for instance
        entities = sorted(entities_map.keys(), key=lambda x, y: len(y) - len(x))
    ...
    def save_output(self, file_name, id=None):
        ...
        """
        formatter = self._get_name_formatter(file_name, self.default_extension, id is not None)
        out_name = formatter(self.default_extension, id)
...
def main(argv):
    ...
    for count, file_path in enumerate(
            file_paths[args.skip:(args.skip + length)]):  # enumerate is used only to set count
